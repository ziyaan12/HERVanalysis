...
# create function to run commands and log the output
run_command() {
  local cmd="$1"
  local log_file="$2"
...
# iterate over all Skmel5-1 files
for sam_file in ${input_dir}/Skmel5-1*_R1+2_001.sam; do
  ...
  if [ -f "${result_subdir}/telescope-TE_counts.tsv" ] && [ -f "${result_subdir}/telescope-run_stats.tsv" ]; then
  ...
  telescope_cmd="telescope assign $sorted_sam_file $gtf_file"
  run_command "$telescope_cmd" "$log_file"
  if [ $? -ne 0 ]; then
  ...
  # move the telescope output files to the result subdirectory
  mv telescope-TE_counts.tsv "${result_subdir}/" || { echo "Failed to move telescope TE counts file for $base_name" >> "$process_log"; }
  mv telescope-run_stats.tsv "${result_subdir}/" || { echo "Failed to move telescope stats file for $base_name" >> "$process_log"; }
